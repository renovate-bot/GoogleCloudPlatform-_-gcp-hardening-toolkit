...
resource "google_org_policy_custom_constraint" "bucket_versioning" {
  name         = "custom.storageBucketVersioning${random_string.constraint_suffix.result}"
  parent       = var.parent
...
resource "time_sleep" "wait_for_constraint_creation" {
  ...
  triggers = {
    constraint_name = google_org_policy_custom_constraint.bucket_versioning.name
  }
}

resource "google_org_policy_policy" "enforce_bucket_versioning" {
  name   = "${var.parent}/policies/${google_org_policy_custom_constraint.bucket_versioning.name}"
  parent = var.parent
